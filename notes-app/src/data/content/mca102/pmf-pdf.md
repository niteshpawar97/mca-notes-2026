...
1. $p(x) \ge 0$ for all $x$ (probability negative nahi ho sakti)
2. $\sum p(x) = 1$ (sabhi possible values ki probabilities ka sum = 1)
...
$$
P(X=0) = P(TT) = \frac{1}{4} \qquad P(X=1) = P(HT) + P(TH) = \frac{1}{4} + \frac{1}{4} = \frac{2}{4} = \frac{1}{2} \qquad P(X=2) = P(HH) = \frac{1}{4}
$$
...
$$
P(a \le X \le b) = \int_a^b f(x)\, dx \qquad \text{(area under the curve between } a \text{ and } b\text{)}
$$

### Properties of PDF

1. $f(x) \ge 0$ for all $x$
2. $\displaystyle\int_{-\infty}^{\infty} f(x)\, dx = 1$ (total area under the curve = 1)
3. $P(X = x) = 0$ for any single exact point $x$ (continuous variable ke liye ek point ki probability hamesha zero hoti hai)

> Warning — Continuous random variable ke liye $P(X = 5)$ hamesha **0** hota hai — sirf ranges ki probability meaningful hoti hai, jaise $P(4 \le X \le 6)$. Ye ek favourite exam trick question hai.
...
$$
f(x) = 2x, \quad \text{for } 0 \le x \le 1 \qquad\qquad f(x) = 0, \quad \text{otherwise}
$$

Verify ki ye valid PDF hai, aur find $P(0.2 \le X \le 0.5)$

**Step 1** — Verify total area = 1:

$$
\int_0^1 2x\, dx = \left[x^2\right]_0^1 = 1 - 0 = 1 \ \checkmark \ \text{(valid PDF)}
$$

**Step 2** — Find $P(0.2 \le X \le 0.5)$:

$$
P(0.2 \le X \le 0.5) = \int_{0.2}^{0.5} 2x\, dx = \left[x^2\right]_{0.2}^{0.5} = (0.5)^2 - (0.2)^2 = 0.25 - 0.04 = 0.21
$$

> Example — PDF questions me integration hi karna padta hai — power rule use karo: $\int x^n\, dx = \dfrac{x^{n+1}}{n+1}$. Simple polynomial PDFs exam me common hote hain.
...
| Sum/Integral = 1 | $\sum p(x) = 1$ | $\int f(x)\,dx = 1$ (over all $x$) |
...
$$
\text{Discrete:}\quad F(x) = P(X \le x) = \sum_{t \le x} p(t)
$$

$$
\text{Continuous:}\quad F(x) = P(X \le x) = \int_{-\infty}^{x} f(t)\, dt
$$
